...
### 표준 튜닝 (4현)
- **G** (1번현, 가장 얇은 줄, 가장 높은 음)
- **D** (2번현)
- **A** (3번현)
- **E** (4번현, 가장 두꺼운 줄, 가장 낮은 음 = E1, 41Hz)

연주 자세에서 **위쪽이 4번 E줄**, 아래쪽이 1번 G줄 (TabView 표기와 같음).

### 오른손 anchor 와 i-m 교대
- 엄지(thumb)는 **픽업 위** 또는 **E줄 위**에 고정 — 손 전체가 안정
- 검지(i) → 중지(m) → 검지(i) → 중지(m) … 8분음 흐름
- **손가락 끝**이 아닌 **살 부분**으로 줄을 걸어 올리고, 다음 줄에 손가락이 **기대는(rest stroke)** 느낌
- 처음엔 느린 BPM(60~80)에서 **균일한 음량**이 최우선. 빠르기는 나중

### 연습 순서
1. 개방현 한 줄(E)에서 i-m 교대 (BPM 60, 4분음 → 8분음)
2. 두 줄 이동하며 교대 (E→A, A→D, D→G)
3. 네 줄 순서대로 (E→A→D→G→D→A→E…)
4. BPM을 5씩 올리며 균일한 음량 유지 확인
5. 메트로놈 강박(1·3박)에 i, 약박(2·4박)에 m을 의식적으로 배치

### Stage 시스템과 연결
이 레슨이 끝나면 **Stage 1 (루트)** 단계로 자연스럽게 이어집니다. ChordTimeline 위 Stage 패널에서 \`루트\`를 선택하면 각 마디 첫 박에만 root 한 음 — 이 레슨에서 익힌 i-m 교대로 짚으면 됩니다.

| Stage | 패턴 | 이 레슨과의 관계 |
|-------|------|-----------------|
| 1 루트 | 첫 박 root | i 한 손가락만 사용 (가장 단순) |
| 2 루트+옥타브 | root–octave | i-m 교대 + 옥타브 stretch |
| 3 루트+5도 | root–5도 | i-m 교대 + 한 현 위 5도 |`,
          },
          drills: [
            {
